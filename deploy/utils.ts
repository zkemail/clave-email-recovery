...
export const verifyEnoughBalance = async (
  ...
  if (balance < amount)
    throw new Error(
      `⛔️ Wallet balance is too low! Required ${ethers.formatEther(
        amount
  ...
  const artifact = await deployer
    ...
    .catch((error) => {
      if (
        ...
        throw new Error(
          `⛔️ Please make sure you have compiled your contracts or specified the correct contract name!`
        );
      } else {
        throw error;
      }
